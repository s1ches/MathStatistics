...
def handle_data(data: DataModel) -> DataModel:
    ...
    data.covariance = ((1/len(data.x_sample)) * sum((data.x_sample - data.x_mean)*(data.y_sample - data.y_mean))
                       / (data.x_std * data.y_std))

    # Коэффициент регрессии
    data.regression_coefficient = data.covariance * data.x_std / data.y_std
